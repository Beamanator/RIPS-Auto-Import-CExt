...
function Utils_StopImport(message, callback) {
    ...
    if (!message) message = "stopping import for unknown reason";
...
function Utils_SkipClient(message, ci) {
    ...
    if (ci == undefined || typeof ci !== "number") ci = "<Unknown>";
    else ci += 1; // -> add 1 for client #, not index
...
function Utils_NavigateToTab(tab_href) {
    if (tab_href !== undefined) $('a[href="' + tab_href + '"]')[0].click();
    else console.warn("Utils_NavigateToTab received invalid tab href");
}
...
function Utils_GetTabHref(tabName) {
    ...
    var map = {
        ...
        "AdvancedSearch-Result":
            "/Stars/SearchClientDetails/ClientListSearchResult"
    };
    ...
    if (tab_href == undefined) console.error("tab name not defined in url map");
    ...
    return new Promise(function(resolve, reject) {
        ...
        var intervalID = setInterval(function() {
            ...
            // check if we've passed the desired amount of iterations on setInterval
                ...
                clearInterval(intervalID);
                reject(
                    `Condition <${Fcondition.name}> never returned true over ` +
...
function Utils_InsertValue(value, id, configParam) {
    ...
    // if value exists, throw into field:
        ...
        // else, we don't need special function
            ...
            if ($elem.length !== 1) success = false;
            else {
                $elem.val(value);
                success = true;
            }
...
function Utils_AddError(message, callback) {
    if (!message || message === "") message = "Unspecified error!";
 ...
 * 6) Next Client - move to next client
 *
...
function Utils_GetNextSearchActionState(action, searchSettings) {
    let nextAction;
    let byUnhcr = searchSettings.byUnhcr,
        byStarsNumber = searchSettings.byStarsNumber;
    let byPhone = searchSettings.byPhone,
        byOtherPhone = searchSettings.byOtherPhone;
    let byPassportNumber = searchSettings.byPassportNumber;
    ...
    switch (action) {
        ...
        case "SEARCH_FOR_CLIENT":
            ...
            else if (byPassportNumber)
                nextAction = "SEARCH_FOR_CLIENT_PASSPORT_NUMBER";
...
function Utils_IsSelectElemPopulated(selector_arr) {
    ...
    if (selector_arr.length > 1)
        console.warn(
            "Utils_IsSelectElemPopulated has too many elements in arg array"
        );
    ...
    if (selectedOptions.length === 0) {
        ...
        console.warn(
            "I think we would be here if more than one option is selected" +
