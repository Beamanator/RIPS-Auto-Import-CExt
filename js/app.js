(function() {
    ...
    function MainController($q, $scope) {
        ...
        // Initial page data
        Ctrl.textareaWelcome =
            'Client details here (delimited by commas "," or tabs)' +
        ...
        Ctrl.importClients = function() {
            ...
            if (Ctrl.headerArr.length === 0 || Ctrl.dataArray.length === 0) {
                let errMessage =
                    `Error: headerArr <${Ctrl.headerArr.length}> or ` +
            ...
            chrome.runtime.sendMessage(mObjFirst, function(response) {
                ...
                chrome.tabs.query(
                    ...
                    function(tabs) {
                        ...
                        if (tabs.length === 0) {
                            let errMessage =
                                "No RIPS tabs are open right now!" +
        ...
        Ctrl.increaseWidth = function(index) {
            Ctrl.widthArray[index] += 20;
        };
        Ctrl.decreaseWidth = function(index) {
            Ctrl.widthArray[index] -= 20;
        };
        Ctrl.getWidth = function(index) {
            return Ctrl.widthArray[index];
        };
        ...
        function convertData(data, delim) {
            ...
            // display errors
            if (errArr.length > 0) displayError(errArr);
